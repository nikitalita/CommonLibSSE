...
namespace RE
{
	...
	public:
		...
		[[gnu::pure]] bool   GetDangerous() const override;                                                        // 1B - { return data.objectReference->GetDangerous(); }
		[[gnu::pure]] bool   GetObstacle() const override;                                                         // 1D - { return data.objectReference ? data.objectReference->GetObstacle() : false; }
		[[gnu::pure]] bool   GetOnLocalMap() const override;                                                       // 1F - { return (flags >> 9) & 1 && data.objectReference->GetOnLocalMap(); }
		[[gnu::pure]] bool   GetMustUpdate() const override;                                                       // 20 - { return data.objectReference->GetMustUpdate(); }
		...
		[[nodiscard]] virtual BGSLocation*                      GetEditorLocation1() const;                                                                                                                                                                                  // 3C - non-pure, possible extraData init
		virtual bool                                            GetEditorLocation2(NiPoint3& a_outPos, NiPoint3& a_outRot, TESForm*& a_outWorldOrCell, TESObjectCELL* a_fallback);                                                                                           // 3D - non-pure, possible extraData init
		...
		[[nodiscard]] virtual bool                              GetFullLODRef() const;                                                                                                                                                                                       // 43 - non-pure, possible Load()
		virtual void                                            SetFullLODRef(bool a_set);                                                                                                                                                                                   // 44
		[[nodiscard]] virtual BGSAnimationSequencer*            GetSequencer() const;                                                                                                                                                                                        // 45 - non-pure, possible extraData init
	  [[gnu::pure]] [[nodiscard]] virtual bool                QCanUpdateSync() const;                                                                                                                                                                                      // 46 - { return true; }
		[[gnu::pure]] [[nodiscard]] virtual bool                GetAllowPromoteToPersistent() const;                                                                                                                                                                         // 47 - { return true; }
		...
		[[nodiscard]] virtual BGSScene*                         GetCurrentScene() const;                                                                                                                                                                                     // 4A - non-pure, possible extraData initialization
		...
		[[nodiscard]] virtual BGSDialogueBranch*                GetExclusiveBranch() const;                                                                                                                                                                                  // 4D - non-pure, possible extraData init
		...
		[[nodiscard]] virtual ActorCause*                       GetActorCause() const;                                                                                                                                                                                       // 51 - non-pure, possible extraData init
		[[nodiscard]] virtual NiPoint3                          GetStartingAngle() const;                                                                                                                                                                                    // 52 - non-pure, possible extraData init
		[[nodiscard]] virtual NiPoint3                          GetStartingLocation() const;                                                                                                                                                                                 // 53 - non-pure, possible extraData init
		...
		[[nodiscard]] virtual NiPoint3                          GetLookingAtLocation() const;                                                                                                                                                                                // 5B - non-pure, runs on another thread
		[[nodiscard]] virtual MagicCaster*                      GetMagicCaster(MagicSystem::CastingSource a_source);                                                                                                                                                         // 5C - non-pure, possible extraData initialization
		[[nodiscard]] virtual MagicTarget*                      GetMagicTarget();                                                                                                                                                                                            // 5D - non-pure, possible extraData initialization
		...
		[[nodiscard]] virtual NiAVObject*                       Get3D1(bool a_firstPerson) const;                                                                                                                                                                            // 6F - { return Get3D2(); }   - none of these are pure, run on another thread
		[[nodiscard]] virtual NiAVObject*                       Get3D2() const;                                                                                                                                                                                              // 70   - none of these are pure, run on another thread
		[[nodiscard]] virtual bool                              Is3rdPersonVisible() const;                                                                                                                                                                                  // 71 - { return true; }
		virtual bool                                            PopulateGraphProjectsToLoad(void) const;                                                                                                                                                                     // 72
		[[nodiscard]] virtual NiPoint3                          GetBoundMin() const;                                                                                                                                                                                         // 73  - none of these are pure, run on another thread
		[[nodiscard]] virtual NiPoint3                          GetBoundMax() const;                                                                                                                                                                                         // 74  - none of these are pure, run on another thread
		...
		[[nodiscard]] virtual const BSTSmartPointer<BipedAnim>& GetBiped1(bool a_firstPerson) const;                                                                                                                                                                         // 7E - { return GetBiped2(); } - none of these are pure, run on another thread
		[[nodiscard]] virtual const BSTSmartPointer<BipedAnim>& GetBiped2() const;                                                                                                                                                                                           // 7F   - none of these are pure, run on another thread
		[[nodiscard]] virtual const BSTSmartPointer<BipedAnim>& GetCurrentBiped() const;                                                                                                                                                                                     // 80 - { return GetBiped2(); }   - none of these are pure, run on another thread
		...
		SKYRIM_REL_VR_VIRTUAL void                         GetLinearVelocity(NiPoint3& a_velocity) const;                                        // 86 - non-pure, runs on another thread
		...
		[[nodiscard]] SKYRIM_REL_VR_VIRTUAL NiNode*        GetFireNode();                                                                        // 8B - { return 0; } - non-pure on Actors
		SKYRIM_REL_VR_VIRTUAL void                         SetFireNode(NiNode* a_fireNode);                                                      // 8C - { return; }
		[[nodiscard]] SKYRIM_REL_VR_VIRTUAL NiAVObject*    GetCurrent3D() const;                                                                 // 8D - { return Get3D2(); } - non-pure, runs on another thread
		...
		[[nodiscard]] SKYRIM_REL_VR_VIRTUAL TESObjectCELL* GetSaveParentCell() const;                                                            // 97 - non-pure, possible ExtraData initialization
		SKYRIM_REL_VR_VIRTUAL void                         SetParentCell(TESObjectCELL* a_cell);                                                 // 98
		[[gnu::pure]] [[nodiscard]] SKYRIM_REL_VR_VIRTUAL bool           IsDead(bool a_notEssential = true) const;                                             // 99
		...
		[[gnu::pure]] [[nodiscard]] SKYRIM_REL_VR_VIRTUAL TESAmmo*       GetCurrentAmmo() const;                                                               // 9E - { return 0; }
		[[nodiscard]] SKYRIM_REL_VR_VIRTUAL BGSDecalGroup* GetDecalGroup() const;                                                                // 9F - non-pure, possible ExtraData initialization
